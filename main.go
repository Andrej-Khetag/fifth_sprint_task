...
func (t Training) distance() float64 {
	...
	if t.TrainingType == "Плавание" {
		return float64(t.Action) * SwimmingLenStep / float64(MInKm)
	}
...
func (r Running) Calories() float64 {
	// вставьте ваш код ниже
	return ((float64(CaloriesMeanSpeedMultiplier)*r.Training.meanSpeed() + CaloriesMeanSpeedShift) * r.Training.Weight / float64(MInKm) * r.Training.Duration.Hours() * float64(MinsInHour))
}
...
func (w Walking) Calories() float64 {
	...
	meanSpeedPow := math.Pow((w.Training.meanSpeed() * KmHInMsec), 2)
	return ((CaloriesWeightMultiplier*w.Training.Weight + (meanSpeedPow/(float64(w.Height)/float64(CmInM)))*CaloriesSpeedHeightMultiplier*w.Training.Weight) * w.Training.Duration.Hours() * float64(MinsInHour))
}
